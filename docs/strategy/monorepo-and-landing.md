...
> **✅ Constraint cleared (updated 2026-06-16):** iOS **1.0.0 LIVE** since 2026-06-15 and **1.1.0 LIVE** (approved and released 2026-06-16). The "don't break fastlane mid-review" gating constraint is gone. **⇒ Phase 2 (iOS absorb) is now unblocked.** Phase 1 (local monorepo scaffold), **Phase 3a (local landing build)**, and **Phase 3b (public deploy on `linkclean.app`)** are all **✅ DONE as of 2026-06-16**. Outstanding: Phase 2 (iOS absorb — schedule between releases, since the next submission window will re-impose the don't-break-fastlane constraint) and Phase 3c (content cadence + ja/de locales, post-launch).
...
| Move the iOS app into `apps/ios/LinkClean/` (Phase 2) | Tidiness + the monorepo end-state | **Real** — Xcode refs, the `LinkCleanKit` SPM relative path, fastlane, scripts, screenshot pipeline | **Unblocked 2026-06-16** (1.0.0 + 1.1.0 both live). Schedule between submission windows — the next submission re-imposes the "don't break fastlane" gate. |
...
**Therefore: do not big-bang.** Phase 1 is **local-only** — stand up the monorepo skeleton + scaffold the landing app so it runs under `wrangler dev`, *while the iOS app stays exactly where it is at the repo root.* Phase 2 (iOS absorb) is unblocked now that 1.1.0 is live; sequence it between submission windows so the next build's fastlane is in a known-good state. Phase 3 now splits into **3a (local content build), 3b (public launch — domain + deploy — ✅ shipped 2026-06-16), and 3c (content cadence + locales)**. Identical end state; sequencing decouples the content build from the infra commitment.
...
### Phase 2 — Absorb the iOS app into `apps/ios/LinkClean/`  *(unblocked 2026-06-16 — schedule between submission windows)*
A single isolated PR doing only the move + the `.gitignore` split + the docs split (§4). No feature work mixed in — keep the diff reviewable and the revert trivial. The gating reason (fastlane mid-review safety) is cleared as of 2026-06-16 (1.1.0 live); execute this **between** the 1.1.0 release and the next submission, so the next build's fastlane is in a known-good state from the start. If a submission is in flight when scheduling lands, wait for that one to clear before merging.
