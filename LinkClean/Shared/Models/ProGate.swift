...
/// and the History window (T1) all agree on the same allowances. Pure value
/// logic — `nonisolated` so it's callable from any context.
nonisolated enum ProGate {
    /// Free users may keep up to this many custom rules; the gate opens past it.
    static let freeCustomRuleAllowance = 1
    ...
    static func canAddCustomRule(entitlement: Entitlement, currentCount: Int) -> Bool {
